...
(+<) v graph = (replace graph $ (reverse $ v:(reverse vs)), Cursor (length vs))
    where vs = values graph
...
(*@<<) :: (GraphContainer g (EdgeT b a rs), Monad m)
      => CursorT a rs -- ^ A cursor related by cursors to be returned.
...
(?<<) f = do
      graph <- get
      return $ L.findIndex f (values graph :: [a]) >>= (Just . Cursor)
